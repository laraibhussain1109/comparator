...
class MainWindow(QMainWindow):
    ...
    def _build(self):
        ...
        layout.addLayout(stats);buttons=QHBoxLayout();self.train=QPushButton("TRAIN");self.start=QPushButton("START INSPECTION");self.stop=QPushButton("STOP INSPECTION");self.stop.setEnabled(False);buttons.addWidget(self.train);buttons.addWidget(self.start);buttons.addWidget(self.stop);layout.addLayout(buttons)
        ...
        self.setCentralWidget(root);self.train.clicked.connect(self.train_models);self.start.clicked.connect(self.start_inspection);self.stop.clicked.connect(self.stop_inspection)
    def _refresh_trained(self):self.start.setEnabled((Path(self.c["paths"]["artifacts"])/"training_summary.json").exists())
    def train_models(self):
        self.train.setEnabled(False);self.start.setEnabled(False);self.progress.setVisible(True);self.progress.setValue(0);self.started=time.monotonic();self.training_thread=QThread();self.worker=TrainingWorker(self.c);self.worker.moveToThread(self.training_thread);self.training_thread.started.connect(self.worker.run);self.worker.progress.connect(self.on_progress);self.worker.completed.connect(self.on_training_done);self.worker.failed.connect(self.on_error);self.worker.finished.connect(self.training_thread.quit);self.worker.finished.connect(lambda:self.train.setEnabled(True));self.training_thread.start()
