...
class Trainer:
    ...
    def run_diagnostics(self)->dict:
        a=Path(self.c["paths"]["artifacts"]);a.mkdir(parents=True,exist_ok=True);inv=scan_dataset(Path(self.c["paths"]["dataset"]));epochs=self.c.get("classifier",{}).get("sanity_epochs",120);result=run_micro_overfit(inv.good,inv.ng,a,PreprocessingConfig(self.c.get("classifier",{}).get("image_size",224)),epochs,lambda epoch,loss,accuracy:self._stage(round(100*epoch/epochs),"Sanity classifier",f"Epoch {epoch}/{epochs} — loss {loss:.6f}, accuracy {accuracy:.1%}"));return {"sanity_check":result,"hardware":hardware_details(),"good_images":len(inv.good),"ng_images":len(inv.ng)}
    def run(self)->dict:
        ...
        sanity_epochs=self.c.get("classifier",{}).get("sanity_epochs",120);sanity=run_micro_overfit(inv.good,inv.ng,a,PreprocessingConfig(self.c.get("classifier",{}).get("image_size",224)),sanity_epochs,lambda epoch,loss,accuracy:self._stage(10+round(6*epoch/sanity_epochs),"Sanity classifier",f"Epoch {epoch}/{sanity_epochs} — loss {loss:.6f}, accuracy {accuracy:.1%}"))
        ...
        classifier=train_classifier(split,a,self.c.get("classifier",{}).get("image_size",224),classifier_epochs,lambda epoch,loss,accuracy:self._stage(18+round(30*epoch/classifier_epochs),"Binary classifier",f"Epoch {epoch}/{classifier_epochs} — loss {loss:.6f}, accuracy {accuracy:.1%}"))
