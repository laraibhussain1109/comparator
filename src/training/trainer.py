...
class Trainer:
    ...
    def run_diagnostics(self)->dict:
        a=Path(self.c["paths"]["artifacts"]);a.mkdir(parents=True,exist_ok=True);inv=scan_dataset(Path(self.c["paths"]["dataset"]));result=run_micro_overfit(inv.good,inv.ng,a,PreprocessingConfig(self.c.get("classifier",{}).get("image_size",224)),self.c.get("classifier",{}).get("sanity_epochs",120));return {"sanity_check":result,"hardware":hardware_details(),"good_images":len(inv.good),"ng_images":len(inv.ng)}
    def run(self)->dict:
        ...
        sanity=run_micro_overfit(inv.good,inv.ng,a,PreprocessingConfig(self.c.get("classifier",{}).get("image_size",224)),self.c.get("classifier",{}).get("sanity_epochs",120))
        ...
        self._stage(18,"Binary classifier","Training frozen pretrained ConvNeXt-Tiny GOOD/NG classifier")
        classifier=train_classifier(split,a,self.c.get("classifier",{}).get("image_size",224),self.c.get("classifier",{}).get("epochs",30))
        ...
        self._stage(72,"Calibration","Calibrating PatchCore thresholds on validation only");vl,vp,vm=score(split.validation);pt,pm=calibrate_threshold(vl,vp)
