...
def evaluate(model,samples,transform,device,threshold=.5,batch_size=2):
    torch,_=require_torch();model.eval();labels=[];ng=[];rows=[]
    with torch.inference_mode():
        for inputs,target,paths in _loader(samples,transform,batch=batch_size):
...
def _train(model,samples,transform,device,epochs,lr,class_weights=None,progress:Callable[[int,float,float],None]|None=None,batch_size=2):
    ...
    if frozen_backbone:
        ...
        model.eval()
        with torch.inference_mode():
            for inputs,labels,_ in _loader(samples,transform,batch=batch_size):
    ...
    for epoch in range(epochs):
        ...
        if frozen_backbone:
            batches=torch.utils.data.DataLoader(feature_dataset,batch_size=batch_size,shuffle=True,pin_memory=torch.cuda.is_available())
            ...
            model.train()
            for inputs,labels,_ in _loader(samples,transform,batch=batch_size,shuffle=True):
...
def run_micro_overfit(good,ng,artifact_dir,preprocessing,epochs=120,progress=None,batch_size=2):
    ...
    log.info("Sanity classifier device=%s hardware=%s input_size=%d batch_size=%d",device,hardware_details(),preprocessing.image_size,batch_size);loss=_train(model,samples,get_eval_transform(preprocessing),device,epochs,1e-3,progress=progress,batch_size=batch_size);path=artifact_dir/"classifier"/"sanity_classifier.pt";save_checkpoint(path,model,preprocessing,epochs,{"sanity":True},.5)
    del model
...
def train_classifier(split,artifact_dir,image_size=224,epochs=30,progress=None,batch_size=2):
    ...
    log.info("Full classifier device=%s hardware=%s input_size=%d batch_size=%d train_images=%d class_counts=%s class_weights=%s",device,hardware_details(),image_size,batch_size,len(split.train),counts,weights);model=create_classifier(True,True).to(device);_train(model,split.train,eval_transform,device,epochs,1e-3,weights,progress,batch_size)
    labels,probs,_=evaluate(model,split.validation,eval_transform,device,batch_size=batch_size);calibrated_threshold,val=calibrate_threshold(labels,probs)
    ...
    path=artifact_dir/"classifier"/"best_classifier.pt";save_checkpoint(path,model,prep,epochs,val,threshold);pre_labels,pre_probs,_=evaluate(model,split.validation,get_eval_transform(prep),device,threshold,batch_size);pre=metrics(pre_labels,pre_probs,threshold)
    del model
    if torch.cuda.is_available():torch.cuda.empty_cache()
    from src.models.classifier_inference import ClassifierInference
